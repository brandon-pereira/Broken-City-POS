...
<%
    ...
    else if(request.getParameter("specialNo") == null)
    {
        response.sendRedirect("specialManager.jsp?error=Invalid special detected, Please try again.");
        return;
    }
    else
...
<html>
    ...
    <body>
        <div class="container">
            <div class="header">
                <div class="breadcrumb"><a href="index.jsp">Home</a> > <a href="specialManager.jsp">Special Manager</a> > Modify Special<div class="right"><a href="../AdminLoginController?logout">Logout</a></div></div>
            ...
            <div class="grid specialEdit">
                <form class="employeeEdit" action="../SpecialController" method="get">
                    <table class="employeeEdit">
                        <input type="hidden" id="specialNo" value="<%= request.getParameter("specialNo") %>" name="specialNo"></td>
                        <tr>
                            <td><label for="specialName">Special Name:</label></td>
                            <td><input type="text" id="specialName" value="<%= helper.getSpecialName(request.getParameter("specialNo")) %>" name="specialName"></td>
                        </tr>
                        <tr>
                            <td><label for="discountedPrice">Discounted Price:</label></td>
                            <td><input type="number" id="discountedPrice" min="0" max="9999" step="0.01" size="4" title="CDA Currency Format - no dollar sign and no comma(s) - cents (.##) are optional" value="<%= helper.getDiscountedPrice(request.getParameter("specialNo")) %>" name="discountPrice"></td>
                     
                        </tr>
                        <tr>
                            <td>Days Of Week:</td>
                            <td>
                                <input type="hidden" name="dow" id="dow" value="<%= helper.getDaysForSpecial(request.getParameter("specialNo")) %>"> 
                                <div class="pill tab">
                        ...
                        <tr>
                            ...
                            <td clas="nopadding">
                                <table class="inner">
                                    <tr>
                                        <td><input type="time" name="startTime" value="<%= helper.getStartTime(request.getParameter("specialNo")) %>"></td>
                                        <td><label for="cost">End Time: </label></td>
                                        <td><input type="time" name="endTime" value="<%= helper.getEndTime(request.getParameter("specialNo")) %>"></td>
                                    </tr>
                                </table>
                            </td>
                        </tr>
            ...
            <div class="specials grid half left">
                <%= helper.getItemsNotOnSpecial(request.getParameter("specialNo")) %>
            </div>
            <div class="specials grid half right">
                <%= helper.getItemsOnSpecial(request.getParameter("specialNo")) %>
            </div>
            ...
            <div class="footer">
                ...
                <div class="right">
                        ...
                        <button class="" name="modify" type="submit">Modify</button>
                </div>
            </form>
        </div>
        </div>
